...
## 📌 About this copy (modified fork)

> **This is a modified copy, not the official TLSNotary release.**
>
> - **Based on:** [`tlsnotary/tlsn` @ `v0.1.0-alpha.14`](https://github.com/tlsnotary/tlsn/tree/v0.1.0-alpha.14)
> - **License:** unchanged — dual-licensed under **MIT OR Apache-2.0** (see the [License](#license) section below). The upstream repository does not ship standalone `LICENSE` files; the license is declared in this README and in each crate's `Cargo.toml`, both preserved here.
> - **What changed:** a **selective-disclosure sweep** was added to the benchmark harness (a new `reveal-recv-percent` parameter) plus the resulting experiment data and analysis. All changes are confined to `crates/harness/`; every other crate is byte-for-byte identical to upstream. The full, file-by-file list of modifications is in **[CHANGELOG.md](./CHANGELOG.md)**.
>
> This fork is not endorsed by or affiliated with the TLSNotary project. The "TLSNotary" name and marks belong to their respective owners.
