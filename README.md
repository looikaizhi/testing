# C2C zkTLS Exchange Protocol · 半去中心化 C2C 兑换协议

[English](#english) | [中文](#中文) · License: Apache-2.0 OR MIT

> 硕士论文配套开源实现：**基于 zkTLS（TLSNotary）的半去中心化 C2C 法币 ↔ 加密货币兑换协议**。
...
一套让 **链下法币支付事实被链上智能合约直接、密码学地验证** 的 C2C 兑换协议。链下用 MPC-TLS（TLSNotary）生成支付证明，验证服务器签名后在链上核验；资产托管与订单状态在链上确定性执行。两大创新：**订单绑定哈希**（防证明跨单复用/篡改）与**半去中心化双域架构**（链上去中心化执行 + 链下受约束公证）。
...
```
前端 dApp ──下单/提交证明──→ 链上合约（托管·状态机·证明核验·风控·保证金）
   ▲                              ▲
   │ 事件订阅                      │ 验证器签名（含订单绑定哈希）
   │                              │
浏览器扩展 ──MPC-TLS──→ 验证服务器 ──核验+签名──┘
   │
   └──→ 支付平台 HTTPS（支付宝/Wise，无需改造）
```
...
> 📖 Full English documentation: **[docs/en/README.md](docs/en/README.md)** (mirrors the Chinese set). This README's English section is a brief overview.
